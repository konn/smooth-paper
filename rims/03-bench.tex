...
\Cref{fig:tower-time-bench,fig:tower-time-bench} show the run-time speed and heap allocation of the existing implementations (\hask{Sparse} and \hask{diffs} only for univariate case), and the proposed method.
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[%
          ...
          legend entries={{\texttt{Sparse}},our method},
          legend to name=leg:time-weil,
          xlabel={Input},ylabel={Alloc (bytes)},%
          title=Identity,%
          symbolic x coords={sparse,x + d,dense},
